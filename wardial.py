...
def wardial(hosts, **kwargs):
    ...
    loop = asyncio.new_event_loop()
    results = loop.run_until_complete(_wardial_async(hosts_copy, **kwargs))
    loop.close()
    num_hosts = len(list(hosts_copy))
    return [hosts_copy[i] for i in range(num_hosts) if results[i]]
